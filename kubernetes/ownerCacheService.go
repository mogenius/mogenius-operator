...
func ControllerForPod(namespace string, podName string) *K8sController {
	...
	if pod == nil {
		K8sLogger.Errorf("Pod: '%s/%s' not found.", namespace, podName)
		return nil
	}
	...
	K8sLogger.Debugf("Pod: '%s/%s' has no owner.", namespace, podName)
	return nil
}
...
func OwnerFromReference(namespace string, ownerRefs []metav1.OwnerReference) *K8sController {
	if len(ownerRefs) > 0 {
		...
		case "Node":
			...
			if err != nil {
				K8sLogger.Errorf("Error getting node: %s", err)
				return nil
			}
			if data != nil {
				if data.OwnerReferences == nil {
			...
			return nil
		default:
			K8sLogger.Errorf("NOT IMPLEMENTED owner kind: %s", owner.Kind)
			return nil
		}
	}
	return nil
