...
func UpdateIngress(ns dtos.K8sNamespaceDto, stage dtos.K8sStageDto, redirectTo *string, skipForDelete *dtos.K8sServiceDto) error {
	...
	logger.Log.Infof("Creating ingress '%s' in namespace '%s'.", *config.Name, *config.Namespace)

	result, err := ingressClient.Apply(context.TODO(), config, applyOptions)
	if err != nil {
		logger.Log.Errorf("UpdateIngress ERROR: %s", err.Error())
	} else {
		logger.Log.Infof("Created ingress '%s' in namespace '%s'.", result.GetObjectMeta().GetName(), result.GetObjectMeta().GetNamespace())
	}
