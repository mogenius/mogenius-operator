...
func GenerateController(namespace dtos.K8sNamespaceDto, service dtos.K8sServiceDto, freshlyCreated bool, client interface{}, handler controllerHandler) (interface{}, error) {
	...
	if len(service.Ports) > 0 {
		spec.Template.Spec.Containers[0].Ports = []core.ContainerPort{}
		for _, port := range service.Ports {
			if port.Expose {
				spec.Template.Spec.Containers[0].Ports = append(spec.Template.Spec.Containers[0].Ports, core.ContainerPort{
					ContainerPort: int32(port.InternalPort),
				})
			}
		}
	} else {
		spec.Template.Spec.Containers[0].Ports = nil
	}
	...
	if service.K8sSettings.IsLimitSetup() {
		...
		spec.Template.Spec.Containers[0].Resources.Limits = limits
		spec.Template.Spec.Containers[0].Resources.Requests = requests
	} else {
		spec.Template.Spec.Containers[0].Resources.Limits = nil
		spec.Template.Spec.Containers[0].Resources.Requests = nil
	}

	spec.Template.Spec.Containers[0].Name = service.Name
	spec.Template.Spec.Containers[0].Command = []string{}
	spec.Template.Spec.Containers[0].Args = []string{}

	// IMAGE
	if service.ContainerImage != "" {
		spec.Template.Spec.Containers[0].Image = service.ContainerImage
		if service.ContainerImageCommand != "" {
			spec.Template.Spec.Containers[0].Command = punqUtils.ParseJsonStringArray(service.ContainerImageCommand)
		}
		if service.ContainerImageCommandArgs != "" {
			spec.Template.Spec.Containers[0].Args = punqUtils.ParseJsonStringArray(service.ContainerImageCommandArgs)
		}
		if service.ContainerImageRepoSecretDecryptValue != "" {
			spec.Template.Spec.ImagePullSecrets = []core.LocalObjectReference{}
			spec.Template.Spec.ImagePullSecrets = append(spec.Template.Spec.ImagePullSecrets, core.LocalObjectReference{
		...
		// 	spec.Template.Spec.Containers[0].Image = previousDeployment.Spec.Template.Spec.Containers[0].Image
		// } else {
		// 	spec.Template.Spec.Containers[0].Image = "ghcr.io/mogenius/mo-default-backend:latest"
		// }
	}

	// ENV VARS
	spec.Template.Spec.Containers[0].Env = []core.EnvVar{}
	spec.Template.Spec.Containers[0].VolumeMounts = []core.VolumeMount{}
	spec.Template.Spec.Volumes = []core.Volume{}
	for _, envVar := range service.EnvVars {
		if envVar.Type == "KEY_VAULT" ||
			envVar.Type == "PLAINTEXT" ||
			envVar.Type == "HOSTNAME" {
			spec.Template.Spec.Containers[0].Env = append(spec.Template.Spec.Containers[0].Env, core.EnvVar{
				Name: envVar.Name,
		...
		if envVar.Type == "VOLUME_MOUNT" {
			...
			if len(components) == 3 {
				...
				spec.Template.Spec.Containers[0].VolumeMounts = append(spec.Template.Spec.Containers[0].VolumeMounts, core.VolumeMount{
				...
				if nfsService != nil {
					...
					if isUnique {
						spec.Template.Spec.Volumes = append(spec.Template.Spec.Volumes, core.Volume{
							Name: volumeName,
	...
	if ContainerSecretDoesExistForStage(namespace) && service.ContainerImageRepoSecretDecryptValue == "" {
		...
		spec.Template.Spec.ImagePullSecrets = []core.LocalObjectReference{}
		spec.Template.Spec.ImagePullSecrets = append(spec.Template.Spec.ImagePullSecrets, core.LocalObjectReference{Name: containerSecretName})
	}
	...
	spec.Template.Spec.Containers[0].SecurityContext = nil
