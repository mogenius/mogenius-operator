...
func LoadDotEnv() {
	...
	if _, err := os.Stat(homeDirName + "/.mogenius-k8s-manager/mogenius-k8s-manager.env"); err == nil || os.IsExist(err) {
		godotenv.Load(homeDirName + "/.mogenius-k8s-manager/mogenius-k8s-manager.env")
	} else {
		...
		if err != nil {
			logger.Log.Fatal("Error writing " + homeDirName + "/.mogenius-k8s-manager/mogenius-k8s-manager.env file")
			logger.Log.Fatal(err)
		}
		godotenv.Load(homeDirName + "/.mogenius-k8s-manager/mogenius-k8s-manager.env")
	}

	tes, readErr := godotenv.Read(homeDirName + "/.mogenius-k8s-manager/mogenius-k8s-manager.env")
	if readErr != nil {
		logger.Log.Fatal("Error reading " + homeDirName + "/.mogenius-k8s-manager/mogenius-k8s-manager.env file")
