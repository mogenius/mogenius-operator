...
func (self *moKubernetes) CreateOrUpdateResourceTemplateConfigmap() error {
	...
	_, err = self.CreateUnstructuredResource("v1", "configmaps", utils.Pointer(self.config.Get("MO_OWN_NAMESPACE")), string(updatedYaml))
	if apierrors.IsAlreadyExists(err) {
		_, err = kubernetes.UpdateUnstructuredResource("v1", "configmaps", self.config.Get("MO_OWN_NAMESPACE"), string(updatedYaml))
		if err != nil {
