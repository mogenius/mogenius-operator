...
func (self *reconciler) findSuperfluousRoleBindings(
	...
	// Build lookup maps from required bindings for O(1) existence checks.
	// The RoleBinding name encodes all managed label fields via an FNV64a hash,
	// so a name match implies label field equality. Subjects are stored separately
	// to handle the case where a User's subject spec changes without affecting the name.
	requiredRBByName := make(map[string]rbacv1.Subject, len(requiredRoleBindings))
	for i := range requiredRoleBindings {
		rb := &requiredRoleBindings[i]
		if len(rb.Subjects) == 1 {
			requiredRBByName[rb.GetName()] = rb.Subjects[0]
		}
	}

	requiredCRBByName := make(map[string]rbacv1.Subject, len(requiredClusterRoleBindings))
	for i := range requiredClusterRoleBindings {
		crb := &requiredClusterRoleBindings[i]
		if len(crb.Subjects) == 1 {
			requiredCRBByName[crb.GetName()] = crb.Subjects[0]
		}
	}
	...
	for _, erb := range existingRoleBindings {
		...
		requiredSubject, found := requiredRBByName[erb.GetName()]
		if !found || !self.subjectsEqual(&requiredSubject, &erb.Subjects[0]) {
	...
	for _, erb := range existingClusterRoleBindings {
		...
		requiredSubject, found := requiredCRBByName[erb.GetName()]
		if !found || !self.subjectsEqual(&requiredSubject, &erb.Subjects[0]) {
...
func (self *reconciler) findMissingRoleBindings(
	...
	// Build lookup maps from existing bindings for O(1) existence checks.
	existingRBByName := make(map[string]rbacv1.Subject, len(existingRoleBindings))
	for i := range existingRoleBindings {
		erb := &existingRoleBindings[i]
		if len(erb.Subjects) == 1 {
			existingRBByName[erb.GetName()] = erb.Subjects[0]
		}
	}

	existingCRBByName := make(map[string]rbacv1.Subject, len(existingClusterRoleBindings))
	for i := range existingClusterRoleBindings {
		erb := &existingClusterRoleBindings[i]
		if len(erb.Subjects) == 1 {
			existingCRBByName[erb.GetName()] = erb.Subjects[0]
		}
	}
	...
	for _, rb := range requiredRoleBindings {
		...
		existingSubject, found := existingRBByName[rb.GetName()]
		if !found || !self.subjectsEqual(&existingSubject, &rb.Subjects[0]) {
	...
	for _, rb := range requiredClusterRoleBindings {
		...
		existingSubject, found := existingCRBByName[rb.GetName()]
		if !found || !self.subjectsEqual(&existingSubject, &rb.Subjects[0]) {
