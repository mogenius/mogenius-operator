...
func parseAiResponse(responseText string) (response *AiResponse, removedText string, err error) {
	...
	if err := json.Unmarshal(responseBytes, &aiResponse); err != nil {
		return nil, removedText, fmt.Errorf("error unmarshaling AI response: %w", err)
	}
	return &aiResponse, removedText, nil
}
...
const submitAnalysisInstruction = "\n\nWhen your investigation is complete, submit your final analysis by calling the " + submitAnalysisToolName + " tool with the complete result. Do not print the final JSON as text — always submit it through the tool."
...
// submitAnalysisAnthropicTool carries the response schema of AiResponse, so
// the final analysis arrives as validated tool input instead of JSON scraped
// out of free text.
var submitAnalysisAnthropicTool = anthropicTool(
	submitAnalysisToolName,
	"Submit your final analysis. Call this exactly once as your last action, after finishing the investigation. The arguments must contain the complete analysis result.",
	map[string]any{
		...
		"analysis": map[string]any{
			...
			"properties": map[string]any{
				...
				"currentResourceYaml": map[string]any{"type": "string", "description": "Current manifest of the target resource (from the cluster)."},
				"targetResourceYaml":  map[string]any{"type": "string", "description": "Complete proposed manifest (required for UpdateResource and CreateResource; omit for DeleteResource)."},
	...
	[]string{"errorMessage", "analysis"},
)

// parseSubmittedAnalysis converts submit_analysis tool input into an
// AiResponse. Kept separate from parseAiResponse: tool input is guaranteed
// JSON, so no text extraction is involved.
func parseSubmittedAnalysis(input json.RawMessage) (*AiResponse, error) {
	var aiResponse AiResponse
	if err := json.Unmarshal(input, &aiResponse); err != nil {
		return nil, fmt.Errorf("error unmarshaling submitted analysis: %w", err)
	}
	if aiResponse.Analysis.ProblemDescription == "" {
		return nil, fmt.Errorf("analysis.problemDescription must not be empty")
	}
