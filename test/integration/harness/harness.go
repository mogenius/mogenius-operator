...
// Use RestCfg to build additional K8s clients for direct API assertions.
type TestHarness struct {
	...
	// RestCfg is the raw envtest REST config for building custom K8s clients.
	RestCfg *rest.Config
...
func New(t *testing.T) *TestHarness {
	...
	// Locate the CRD YAML directory relative to this source file so the path
	// is stable regardless of where the test binary is invoked from.
	_, thisFile, _, _ := runtime.Caller(0)
	crdDir := filepath.Join(filepath.Dir(thisFile), "../../../src/crds/yaml")
...
// WaitFor polls pattern+payload on APIServer until check(response) returns true.
// It fails the test if the condition is not satisfied within 15 seconds.
// The last matching Datagram is returned so callers can inspect it without a second Send.
func (h *TestHarness) WaitFor(ctx context.Context, t *testing.T, pattern string, payload any, check func(Datagram) bool) Datagram {
	t.Helper()
	var last Datagram
	require.Eventually(t, func() bool {
		r, err := h.APIServer.Send(ctx, pattern, payload)
		if err != nil {
			return false
		}
		last = r
