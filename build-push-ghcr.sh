...
DOCKER_BUILDKIT=1 docker build --platform linux/amd64 $DOCKER_TAGS_ARGS --build-arg NEXT_VERSION="$NEXT_VERSION" --build-arg BUILD_TIMESTAMP="$BUILD_TIMESTAMP" --build-arg GIT_BRANCH="$GIT_BRANCH" --build-arg COMMIT_HASH="$COMMIT_HASH" -f Dockerfile .
